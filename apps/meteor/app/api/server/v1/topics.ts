...
declare module '@rocket.chat/rest-typings' {
	...
	interface Endpoints {
		...
		'/v1/topics.discussion.detail': {
			GET: (params: { topicId: string; drid: string; offset1?: number; offset2?: number; direction?: string; limit?: number }) => {
				topic: {
					// _id: string;
					rid: string;
					drid: string;
...
API.v1.addRoute(
	...
	{
		async get() {
			const { topicId, drid, offset1, offset2, direction, limit = DEFAULT_PAGE_SIZE } = this.queryParams;
			check(topicId, String);
			check(drid, String);
			// if (offset1) {
			// 	check(offset1, Number);
			// }
			// if (offset2) {
			// 	check(offset2, Number);
			// }
			if (direction) {
			...
			if (direction === 'first') {
				...
				findOptions.sort = { ts: 1 as SortDirection };
				query.ts = { $gt: new Date(offset1) };
			} else if (direction === 'next') { // 下一页
				query.ts = { $lt: new Date(offset2) };
			}
			// 获取评论列表
			let comments = await Messages.find(query, findOptions).toArray();
			// 上一页时要翻转结果
			if (direction === 'prev') {
				comments.reverse();
			}
			...
			if (comments.length === 0) {
				// 原页内容返回, offset1~offset2之间的内容
				query.ts = { $lte: new Date(offset1), $gte: new Date(offset2) };
				findOptions.sort = { ts: -1 as SortDirection };
